...
from reid.evaluators import extract_st_features
...
def get_distmat(model,data_loader,query,gallery,metric,filter = False):
    ...
    """
    features, _ = extract_st_features(model, data_loader)
...
def main(args):
    ...
    model = resnet_softmax_triplet.resnet50_s_t(pretrained=False, num_classes=2420)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Self training answer")
    # data
    ...
    parser.add_argument('--save-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, 'vis_space_src_combined'))
