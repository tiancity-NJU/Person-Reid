    ...
    def download(self):
        ...
        def register(subdir, pattern=re.compile(r'([-\d]+)_c(\d)')):
            ...
            for fpath in fpaths:
                ...
                if subdir == 'query': query_imgs.append(fname)

            if subdir == 'query': return pids,query_imgs
            return pids

        trainval_pids = register('bounding_box_train')
        gallery_pids = register('bounding_box_test')
        query_pids,query_imgs = register('query')
        ...
        query={'query':query_imgs}
        write_json(query,osp.join(self.root,'query.json'))
