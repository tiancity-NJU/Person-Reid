...
def creat_resnet(depth,pretrained=False,**kwargs):
    """

    :param depth: The depth of resnet
    :return: The (pretrained) resnet model
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], **kwargs)
    if depth == 18:
        model = ResNet(BasicBlock, [2, 2, 2, 2], **kwargs)
    ...
    else: raise RuntimeError('illegal depth')
...
class PCB_resnet(nn.Module):
    def __init__(self,arch='resnet50',last_conv_stride = 1,last_conv_dilation = 1,
                 num_stripes = 6,local_conv_out_channel = 256,num_class = 0):

        super(PCB_resnet,self).__init__()
        assert arch in __base__

        self.base = creat_resnet(__base__[arch],pretrained=True,last_conv_stride=last_conv_stride,last_conv_dilation=last_conv_dilation)
