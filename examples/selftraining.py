...
from reid.models import resnet_softmax_triplet
from reid.loss import SelfTraining_TripletLoss
from reid.trainers import Trainer_softmax_triplet
from reid.evaluators import Evaluator_softmax_triplet, extract_st_features
...
def main(args):
    ...
    model = resnet_softmax_triplet.resnet50_s_t(pretrained=False, num_classes=src_num_classes)
    ...
    # Evaluator
    evaluator = Evaluator_softmax_triplet(model)
    ...
    # Criterion
    criterion = [
        SelfTraining_TripletLoss(args.margin, args.num_instances).cuda(),
        SelfTraining_TripletLoss(args.margin, args.num_instances).cuda(),
    ]
    ...
    for iter_n in range(args.iteration):
        if args.lambda_value == 0:
            ...
            source_features, _ = extract_st_features(model, src_extfeat_loader)
        ...
        target_features, _ = extract_st_features(model, tgt_extfeat_loader)
        ...
        trainer = Trainer_softmax_triplet(model, criterion)
        evaluator = Evaluator_softmax_triplet(model)
        # Start training
