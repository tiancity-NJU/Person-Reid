...
from reid.evaluators import Evaluator_softmax_triplet
...
def main(args):
    ...
    model = resnet_softmax_triplet.resnet50_s_t(pretrained=True, num_classes=num_classes)
    ...
    if args.resume:
        ...
        try:
            ...
            print('only could load part model ! !')
    ...
    criterions = []
    criterions.append(TripletLoss().cuda())
    criterions.append(nn.CrossEntropyLoss().cuda())

    # criterions = [
    #     SelfTraining_TripletLoss(args.margin, args.num_instances,use_semi=False).cuda(),
    #     SelfTraining_TripletLoss(args.margin, args.num_instances,use_semi=False).cuda(),
    # ]
    ...
    # Trainer
    trainer = Trainer_softmax_triplet(model, criterions)
